...
export class TundraSnowflakesEffect {
  ...
  constructor(planetRadius: number, params: TundraSnowflakesParams = {}) {
    ...
    this.verticalOscillation = this.rng.uniform(0.1, 0.4); // Vertical movement amplitude
...
/**
 * Función específica para crear partículas de polvo de carbón para planetas Carbon
 */
export function createCarbonDustParticlesFromPythonData(planetRadius: number, surfaceData: any, globalSeed?: number): TundraSnowflakesEffect | null {
  ...
  // Colores más visibles para contraste contra planeta negro
  const carbonDustColors = [
    new THREE.Color(0.8, 0.7, 0.6), // Beige claro (polvo)
    new THREE.Color(0.9, 0.8, 0.7), // Crema (polvo iluminado)
    new THREE.Color(0.6, 0.5, 0.4), // Marrón claro (polvo denso)
    new THREE.Color(1.0, 0.9, 0.8), // Casi blanco (polvo fino)
    new THREE.Color(0.7, 0.6, 0.5), // Gris marrón (polvo medio)
  ];

  const particleCount = 15; // Menos partículas pero muy visibles
  const windSpeed = 2.5; // Velocidad media para movimiento natural

  return new TundraSnowflakesEffect(planetRadius, {
    particleCount,
    windSpeed,
    size: 1.5, // Tamaño más grande para mejor visibilidad
