...
export class PlanetLayerSystem {
  ...
  /**
   * Crea un material para LavaFluid que funciona como capa (similar a AquiferWater pero más lento)
   */
  createLavaFluidLayerMaterial(params: any): THREE.ShaderMaterial {
    const vertexShader = `
      uniform float time;
      uniform float waveHeight;
      uniform float waveFrequency;
      ...
      float noise(vec3 st) {
        ...
        float top = mix(mix(e, f1, u.x), mix(g, h, u.x), u.y);
      ...
      void main() {
        ...
        // Calcular olas muy lentas y viscosas para lava
        vec3 pos = position + vec3(seedOffset);
        
        // Ondas primarias - muy lentas
        float wave1 = sin(pos.x * waveFrequency + time * waveSpeed + seedOffset) * 
                     cos(pos.y * waveFrequency * 0.8 + time * waveSpeed * 0.6 + seedOffset * 1.2) * 
                     waveHeight;
        
        // Ondas secundarias - aún más lentas y de menor amplitud
        float wave2 = sin(pos.z * waveFrequency * 1.3 + time * waveSpeed * 0.3 + seedOffset * 1.5) * 
                     cos(pos.x * waveFrequency * 0.6 + time * waveSpeed * 0.2 + seedOffset * 0.8) * 
                     waveHeight * 0.4;
        
        // Ruido para viscosidad - movimientos muy lentos y orgánicos
        float viscousNoise = noise(pos * 3.0 + vec3(time * waveSpeed * 0.1)) * waveHeight * 0.2;
        
        vWaveHeight = wave1 + wave2 + viscousNoise;
        
        // Aplicar el desplazamiento a la posición
        vec3 newPosition = position + normal * vWaveHeight;
    ...
    const fragmentShader = `
      ...
      uniform vec3 hotLavaColor;
      uniform vec3 coolingLavaColor;
      ...
      // Ruido para variaciones de temperatura
      float random(vec3 st) {
        return fract(sin(dot(st.xyz, vec3(12.9898, 78.233, 54.321))) * 43758.5453123);
      }
      ...
      void main() {
        ...
        // Rim lighting para efecto volcánico
        float rimLight = 1.0 - abs(dotNL);
        rimLight = pow(rimLight, 2.0) * 0.2;
        
        // Variación de temperatura basada en posición y tiempo (muy lenta)
        vec3 pos = vPosition + vec3(seedOffset);
        float temperatureNoise = noise(pos * 2.0 + vec3(time * 0.05)) * 0.5 + 0.5;
        
        // Color base de lava con variación por elevación y temperatura
        float elevation = (vWaveHeight + 1.0) * 0.5; // Normalizar a 0-1
        vec3 baseColor = mix(coolingLavaColor, lavaColor, elevation);
        
        // Zonas más calientes en crestas y según ruido de temperatura
        float heatFactor = max(elevation, temperatureNoise) * 0.7 + temperatureNoise * 0.3;
        baseColor = mix(baseColor, hotLavaColor, heatFactor);
        
        // Calcular especular metálico
        vec3 viewDir = normalize(cameraPosition - vWorldPosition);
        vec3 halfwayDir = normalize(lightDir + viewDir);
        float NdotH = max(dot(normal, halfwayDir), 0.0);
        float specularStrength = pow(NdotH, mix(8.0, 32.0, 1.0 - roughness)) * specularIntensity;
        
        // Aplicar iluminación con componente emisiva
        float totalLight = ambientStrength + (lightIntensity * dayNight) + rimLight;
        vec3 finalColor = baseColor * totalLight;
        
        // Añadir emisión constante (la lava brilla por sí misma)
        finalColor += baseColor * emissiveIntensity * heatFactor;
        
        // Añadir especular SOLO en la parte iluminada
        finalColor += vec3(1.0, 0.8, 0.4) * specularStrength * dayNight; // Especular cálido para lava
        
        gl_FragColor = vec4(finalColor, 1.0 - transparency);
      }
    `;

    const lavaColor = params.lavaColor instanceof THREE.Color ? 
      params.lavaColor : new THREE.Color(params.lavaColor || 0xFF4500);
    const hotLavaColor = params.hotLavaColor instanceof THREE.Color ? 
      params.hotLavaColor : new THREE.Color(params.hotLavaColor || 0xFF6600);
    const coolingLavaColor = params.coolingLavaColor instanceof THREE.Color ? 
      params.coolingLavaColor : new THREE.Color(params.coolingLavaColor || 0x8B0000);
    ...
    return new THREE.ShaderMaterial({
      ...
      uniforms: {
        ...
        // Offset basado en seed para ondas únicas por planeta
        seedOffset: { value: (params.seed || 0) % 100 },
        // Parámetros de olas - mucho más lentos que agua
        waveHeight: { value: params.waveHeight || 0.05 },
        waveFrequency: { value: params.waveFrequency || 2.0 },
        waveSpeed: { value: params.waveSpeed || 0.1 },  // Muy lento para viscosidad
        // Colores de lava
        lavaColor: { value: lavaColor },
        hotLavaColor: { value: hotLavaColor },
        coolingLavaColor: { value: coolingLavaColor },
        // Efectos visuales
        emissiveIntensity: { value: params.emissiveIntensity || 0.5 },
        specularIntensity: { value: params.specularIntensity || 2.0 },
        transparency: { value: params.transparency || 0.05 }, // Menos transparente que agua
        roughness: { value: params.roughness || 0.7 },
        // Uniformes de luz
        lightDirection: { value: new THREE.Vector3(1, 1, 1).normalize() },
        lightPosition: { value: new THREE.Vector3(0, 0, 0) },
        ambientStrength: { value: 0.2 }, // Más luz ambiente para emisión
