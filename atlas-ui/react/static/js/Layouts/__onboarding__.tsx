...
const OnboardingLayout: React.FC<OnboardingLayoutProps> = ({ version }) => {
  ...
  const handleAnimationComplete = () => {
    ...
    setTimeout(() => {
      ...
      form.submit();
    }, 1000); // 5 seconds delay for dramatic effect
  };
  ...
  return (
    <>
      ...
      {/* Loading overlay for redirect state */}
      {animationState === "redirecting" && (
        <div className="fixed inset-0 bg-black z-50 flex items-center justify-center">
          <div className="text-center space-y-4">
            <div className="w-16 h-16 border-4 border-cyan-400 border-t-transparent rounded-full animate-spin mx-auto"></div>
            <p className="text-cyan-400 text-xl font-semibold">Atlas Initialization Protocol Finished</p>
          </div>
        </div>
      )}
    </>
