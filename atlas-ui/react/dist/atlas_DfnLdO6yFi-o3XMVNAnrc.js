import{B as U,a as v,S as X,N as $,P as tt}from"./atlas_CqUMNY_RyRklhCsbOI68T.js";class z{seed;constructor(e){this.seed=e}next(){return this.seed=this.seed*16807%2147483647,(this.seed-1)/2147483646}uniform(e,t){return e+(t-e)*this.next()}choice(e){return e[Math.floor(this.next()*e.length)]}}class et{ringSystem;material;params;planetRadius;constructor(e,t){this.planetRadius=e,this.params=t,this.createRingSystemFromAPI(t)}createRingSystemFromAPI(e){const{full_ring:t,ontop_ring:c,ring_inner_radius:o,ring_outer_radius:l,tilt_factor:_,planet_radius:f,shape_seed:d}=e;if(!t||!c)return;const m=[...t.particles,...c.particles],h=m.length,x=new z(d||12345),u=new U,g=new Float32Array(h*3),p=new Float32Array(h*3),S=new Float32Array(h),F=[{baseGray:.18,variation:.04,name:"dark"},{baseGray:.25,variation:.06,name:"medium"},{baseGray:.32,variation:.06,name:"light"},{baseGray:.25,variation:.08,name:"mixed"}],b=x.choice(F);for(let i=0;i<h;i++){const r=m[i],w=this.planetRadius/(f||200),V=(d||12345)+i,n=new z(V),M=r.distance*w,A=r.angle,P=M*Math.sin(A),R=Math.asin((_||.2)*.5),T=P*Math.sin(R),k=P*Math.cos(R),s=((l||400)-(o||200))*w*.4,N=n.uniform(-s*.8,s*.8),B=n.uniform(-s*.3,s*.3),D=n.uniform(-.08,.08),I=M+B,O=A+D;g[i*3]=I*Math.cos(O),g[i*3+1]=T+N+this.planetRadius*.15,g[i*3+2]=k+n.uniform(-s*.4,s*.4),r.color[0]/255;const j=(r.distance-(o||200))/((l||400)-(o||200)),E=b.baseGray,G=b.variation,Z=n.uniform(-G,G),q=Math.max(.12,Math.min(.45,E+Z)),W=.8+j*.4,Y=n.uniform(.85,1.15),C=n.uniform(0,1),H=C<.03?n.uniform(1.1,1.3):1,J=q*W*Y*H,y=Math.max(.1,Math.min(.55,J));p[i*3]=y,p[i*3+1]=y,p[i*3+2]=y;const K=.15,L=n.uniform(.3,.7),Q=C<.1?n.uniform(1.05,1.2):1;S[i]=r.size*K*L*Q}u.setAttribute("position",new v(g,3)),u.setAttribute("color",new v(p,3)),u.setAttribute("size",new v(S,1)),this.material=new X({uniforms:{brightness:{value:2.2}},vertexShader:`
