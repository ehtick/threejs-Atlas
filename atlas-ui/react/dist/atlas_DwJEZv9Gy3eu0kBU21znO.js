import{S as x,g as y,D as P}from"./atlas_CZFakru7GGKiPV4DHwGyu.js";import{C as S,S as C,A as E,N as T,F as R,M as b,d as _,V as D}from"./atlas_BjkcO1_mNssSqp05cLhjd.js";const i={SIZE:{min:.12,max:.2},ROTATION_SPEED:{min:.05,max:.1},OPACITY:{min:.15,max:.35},TIME_SPEED:{min:.8,max:1.5}},O=`
  varying vec2 vUv;
...
`;class w{mesh;material;params;startTime;proceduralParams;constructor(e){const n=e.seed||Math.floor(Math.random()*1e6),t=new x(n);this.startTime=e.startTime||n%1e4/1e3,this.proceduralParams={size:t.uniform(i.SIZE.min,i.SIZE.max),rotationSpeed:t.uniform(i.ROTATION_SPEED.min,i.ROTATION_SPEED.max),opacity:t.uniform(i.OPACITY.min,i.OPACITY.max),timeSpeed:t.uniform(i.TIME_SPEED.min,i.TIME_SPEED.max)},this.params=e;const o=new S(e.planetColor),r=o.clone();r.multiplyScalar(1-e.hexagonData.color_darken_factor),this.material=new C({uniforms:{time:{value:0},planetColor:{value:o},hexagonColor:{value:r},darkenFactor:{value:e.hexagonData.color_darken_factor},opacity:{value:this.proceduralParams.opacity},hexagonRadius:{value:this.proceduralParams.size},rotationSpeed:{value:this.proceduralParams.rotationSpeed},pole:{value:e.hexagonData.pole==="north"?1:-1},visibility:{value:1}},vertexShader:O,fragmentShader:I,transparent:!0,depthWrite:!1,side:R,blending:e.hexagonData.nebula_blend?E:T});const s=this.createCurvedHexagonGeometry(e.hexagonData.pole,e.hexagonData.radius);this.mesh=new b(s,this.material),this.mesh.scale.set(e.planetRadius,e.planetRadius,e.planetRadius),this.updateVisibility()}updateVisibility(){if(!this.params.hexagonData.enabled){this.material.uniforms.visibility.value=0;return}const n=(this.params.currentTime||0)%this.params.hexagonData.cycle_duration_years/this.params.hexagonData.cycle_duration_years,t=this.params.hexagonData.visible_duration_years/this.params.hexagonData.cycle_duration_years;if(n<t){const o=n/t;o<.1?this.material.uniforms.visibility.value=o/.1:o>.9?this.material.uniforms.visibility.value=(1-o)/.1:this.material.uniforms.visibility.value=1}else this.material.uniforms.visibility.value=0}update(e){const t=y(P,this.proceduralParams.timeSpeed,this.startTime);this.material.uniforms.time.value=t,this.updateVisibility()}addToScene(e){e.add(this.mesh)}removeFromScene(e){e.remove(this.mesh)}dispose(){this.material.dispose(),this.mesh.geometry.dispose()}createCurvedHexagonGeometry(e,n){const t=e==="north"?1:-1,o=64,r=1,s=new _(r,r,o,o),c=s.attributes.position,a=new D;for(let d=0;d<c.count;d++){a.fromBufferAttribute(c,d);const m=a.x,h=a.y,l=Math.sqrt(m*m+h*h);if(l<=r/2){const u=l*Math.PI*.5,p=t*Math.cos(u)*1.02,v=Math.sin(u)*1.02;if(l>0){const f=m/l,g=h/l;a.x=f*v,a.y=p,a.z=g*v}else a.x=0,a.y=t*1.02,a.z=0}c.setXYZ(d,a.x,a.y,a.z)}return c.needsUpdate=!0,s.computeVertexNormals(),s}setEnabled(e){this.mesh.visible=e}}export{w as P};
