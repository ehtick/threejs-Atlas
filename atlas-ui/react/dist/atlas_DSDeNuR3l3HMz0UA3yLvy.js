import{B as E,P,a as c,S as T,A as y}from"./atlas_BjkcO1_mNssSqp05cLhjd.js";class f{seed;constructor(t){this.seed=t}random(){return this.seed=(this.seed*9301+49297)%233280,this.seed/233280}uniform(t,e){return t+this.random()*(e-t)}randint(t,e){return Math.floor(this.random()*(e-t+1))+t}spherePosition(t){const e=this.random()*Math.PI*2,s=Math.acos(this.random()*2-1);return{x:t*Math.sin(s)*Math.cos(e),y:t*Math.sin(s)*Math.sin(e),z:t*Math.cos(s)}}colorVariation(t,e=.4){return{r:t.r*(.8+this.random()*e),g:t.g*(.8+this.random()*e),b:t.b*(.8+this.random()*e)}}}function C(r){return Date.now()/1e3-r}function A(r,t=1,e=0){const s=C(r);return e+s*t}const g=51408e4,a={PARTICLE_COUNT:{min:60,max:150},PHASE_INTENSITY:{min:.4,max:.9},TRANSITION_SPEED:{min:1,max:4},COHERENCE_LEVEL:{min:.2,max:.7},TIME_SPEED:{min:.6,max:2.2},PHASE_STATES:{min:3,max:6}};class h{phaseSystem;material;geometry;params;particleCount;startTime;static vertexShader=`
