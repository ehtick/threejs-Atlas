import{S as Z}from"./atlas_CGrAwTYnSDNTtni2msvpu.js";import{C as W,G as rt,Y as I,R as N,w as R,S as V,Z as nt,_ as lt,$ as ct,a0 as ht,D as mt,V as y,d as ut,g as q,M as pt,f as ft}from"./atlas_BptVXGp7hwWSDe7IbaYgj.js";const S={WATER_BODY_COUNT:{min:4,max:8},WATER_BODY_RADIUS:{min:.07,max:.35},WATER_BODY_OPACITY:{min:.6,max:.85}};class dt{waterGroup;waterBodyMeshes=[];params;rng;materials=[];planetRadius;waterMaterial;normalMap;displacementMap;foamMap;animationTime=0;constructor(e,t={}){const s=t.seed||Math.floor(Math.random()*1e6);this.rng=new Z(s),this.planetRadius=e,this.params={water_bodies:t.water_bodies||[],waterColor:t.waterColor||new W(.2,.5,.9),globalIrregularity:t.globalIrregularity||.7,seed:s,...t},this.waterGroup=new rt,this.generateWaterTextures(),this.createUnifiedWaterMaterial(),this.generateWaterBodies()}generateWaterTextures(){const t=new Uint8Array(262144);for(let l=0;l<256;l++)for(let a=0;a<256;a++){const n=(l*256+a)*4,u=l/256*Math.PI*6,c=a/256*Math.PI*6,M=Math.sin(u*1+c*.5)*.5,f=Math.sin(u*2.3-c*1.1)*.3,h=Math.sin(u*.7+c*1.9)*.2,g=(M+f)*.5+.5,x=(f+h)*.5+.5,z=1;t[n]=g*255,t[n+1]=x*255,t[n+2]=z*255,t[n+3]=255}this.normalMap=new I(t,256,256,N),this.normalMap.wrapS=R,this.normalMap.wrapT=R,this.normalMap.needsUpdate=!0;const s=128,p=new Uint8Array(s*s*4);for(let l=0;l<s;l++)for(let a=0;a<s;a++){const n=(l*s+a)*4,u=l/s*Math.PI*2,c=a/s*Math.PI*2,M=Math.sin(u*2+c)*.3,f=Math.cos(u-c*2)*.3,h=(M+f)*.25+.5,g=Math.max(0,Math.min(255,h*255));p[n]=g,p[n+1]=g,p[n+2]=g,p[n+3]=255}this.displacementMap=new I(p,s,s,N),this.displacementMap.wrapS=R,this.displacementMap.wrapT=R,this.displacementMap.needsUpdate=!0;const r=128,i=new Uint8Array(r*r*4);for(let l=0;l<r;l++)for(let a=0;a<r;a++){const n=(l*r+a)*4,u=l/r*Math.PI*6,c=a/r*Math.PI*6,M=(Math.sin(u)*Math.cos(c)+1)*.5,h=(M>.6?M*.8:0)*255;i[n]=h,i[n+1]=h,i[n+2]=h,i[n+3]=255}this.foamMap=new I(i,r,r,N),this.foamMap.wrapS=R,this.foamMap.wrapT=R,this.foamMap.needsUpdate=!0}createUnifiedWaterMaterial(){const e=`
      uniform float time;
