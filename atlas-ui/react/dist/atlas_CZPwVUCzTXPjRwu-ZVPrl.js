import{B as E,P,a as r,S as y,A as T}from"./atlas_DoxhO7agDNvhtnb7c3WZw.js";class p{seed;constructor(a){this.seed=a}random(){return this.seed=(this.seed*9301+49297)%233280,this.seed/233280}uniform(a,e){return a+this.random()*(e-a)}randint(a,e){return Math.floor(this.random()*(e-a+1))+a}spherePosition(a){const e=this.random()*Math.PI*2,s=Math.acos(this.random()*2-1);return{x:a*Math.sin(s)*Math.cos(e),y:a*Math.sin(s)*Math.sin(e),z:a*Math.cos(s)}}colorVariation(a,e=.4){return{r:a.r*(.8+this.random()*e),g:a.g*(.8+this.random()*e),b:a.b*(.8+this.random()*e)}}}const t={PARTICLE_COUNT:{min:60,max:150},PHASE_INTENSITY:{min:.4,max:.9},TRANSITION_SPEED:{min:1,max:4},COHERENCE_LEVEL:{min:.2,max:.7},TIME_SPEED:{min:.6,max:2.2},PHASE_STATES:{min:3,max:6}};class l{phaseSystem;material;geometry;params;particleCount;startTime;static vertexShader=`
  ...
  `;constructor(a,e={}){const s=e.seed||Math.floor(Math.random()*1e6),i=new p(s);this.startTime=e.startTime||s%1e4/1e3,this.params={particleCount:e.particleCount||Math.floor(i.uniform(t.PARTICLE_COUNT.min,t.PARTICLE_COUNT.max)),phaseIntensity:e.phaseIntensity||i.uniform(t.PHASE_INTENSITY.min,t.PHASE_INTENSITY.max),transitionSpeed:e.transitionSpeed||i.uniform(t.TRANSITION_SPEED.min,t.TRANSITION_SPEED.max),coherenceLevel:e.coherenceLevel||i.uniform(t.COHERENCE_LEVEL.min,t.COHERENCE_LEVEL.max),timeSpeed:e.timeSpeed||i.uniform(t.TIME_SPEED.min,t.TIME_SPEED.max),phaseStates:e.phaseStates||Math.floor(i.uniform(t.PHASE_STATES.min,t.PHASE_STATES.max)),seed:s,startTime:this.startTime},this.particleCount=this.params.particleCount,this.geometry=new E,this.material=this.createMaterial(),this.generatePhaseParticles(a),this.phaseSystem=new P(this.geometry,this.material)}generatePhaseParticles(a){const e=new Float32Array(this.particleCount*3),s=new Float32Array(this.particleCount),i=new Float32Array(this.particleCount*3),c=new Float32Array(this.particleCount),u=new Float32Array(this.particleCount),d=new Float32Array(this.particleCount),S=this.params.seed||Math.floor(Math.random()*1e6),n=new p(S);for(let o=0;o<this.particleCount;o++){const v=a*n.uniform(1.1,1.9),h=n.spherePosition(v);e[o*3]=h.x,e[o*3+1]=h.y,e[o*3+2]=h.z,s[o]=n.uniform(.8,2);const m=n.spherePosition(1);i[o*3]=m.x,i[o*3+1]=m.y,i[o*3+2]=m.z,c[o]=n.uniform(.1,1),u[o]=n.uniform(0,this.params.phaseStates),d[o]=n.uniform(0,Math.PI*2)}this.geometry.setAttribute("position",new r(e,3)),this.geometry.setAttribute("size",new r(s,1)),this.geometry.setAttribute("phaseVector",new r(i,3)),this.geometry.setAttribute("coherenceFactor",new r(c,1)),this.geometry.setAttribute("phaseState",new r(u,1)),this.geometry.setAttribute("transitionPhase",new r(d,1))}createMaterial(){return new y({vertexShader:l.vertexShader,fragmentShader:l.fragmentShader,uniforms:{time:{value:0},phaseIntensity:{value:this.params.phaseIntensity},transitionSpeed:{value:this.params.transitionSpeed},coherenceLevel:{value:this.params.coherenceLevel},phaseStates:{value:this.params.phaseStates}},transparent:!0,blending:T,depthWrite:!1})}addToScene(a,e){e&&this.phaseSystem.position.copy(e),a.add(this.phaseSystem)}update(){const e=(this.startTime+Date.now()/1e3*this.params.timeSpeed)%1e3;this.material.uniforms.time.value=e,this.phaseSystem.rotation.x=e*.012*Math.cos(e*.3),this.phaseSystem.rotation.y=e*.008*Math.sin(e*.5),this.phaseSystem.rotation.z=e*.006*Math.cos(e*.7)}getObject3D(){return this.phaseSystem}dispose(){this.geometry.dispose(),this.material.dispose()}}function A(f,a,e){const s=e||Math.floor(Math.random()*1e6),i=new p(s+9e3),c={particleCount:Math.floor(i.uniform(t.PARTICLE_COUNT.min,t.PARTICLE_COUNT.max)),phaseIntensity:i.uniform(t.PHASE_INTENSITY.min,t.PHASE_INTENSITY.max),transitionSpeed:i.uniform(t.TRANSITION_SPEED.min,t.TRANSITION_SPEED.max),coherenceLevel:i.uniform(t.COHERENCE_LEVEL.min,t.COHERENCE_LEVEL.max),timeSpeed:i.uniform(t.TIME_SPEED.min,t.TIME_SPEED.max),phaseStates:Math.floor(i.uniform(t.PHASE_STATES.min,t.PHASE_STATES.max)),seed:s,startTime:s%1e4/1e3};return new l(f,c)}export{l as A,p as S,A as c};
