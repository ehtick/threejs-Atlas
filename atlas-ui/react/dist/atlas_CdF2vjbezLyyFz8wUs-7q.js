import{D as f,S as _}from"./atlas_DhOreEVM1uBHzRPTt7uMw.js";import{G as w,V as N,B as O,a as p,S as A,C as v,D as M,A as C,n as D}from"./atlas_CLp6T-BwF8hBVTNM0OiQd.js";const t={RING_COUNT:{min:16,max:256},EXPANSION_SPEED:{min:1.5,max:5},WAVE_INTENSITY:{min:.6,max:2},MAX_RADIUS_MULTIPLIER:{min:1.5,max:10},GLOW_INTENSITY:{min:.5,max:2},TIME_SPEED:{min:.8,max:1.3}};class b{group;concentricRings=[];params;rng;planetRadius;cosmicOffset;constructor(i,n={}){this.planetRadius=i;const e=n.seed||Math.floor(Math.random()*1e6);this.rng=new _(e),this.params={color:n.color||[.3,1,.2],ringCount:n.ringCount||Math.floor(this.rng.random()*(t.RING_COUNT.max-t.RING_COUNT.min)+t.RING_COUNT.min),expansionSpeed:n.expansionSpeed||this.rng.random()*(t.EXPANSION_SPEED.max-t.EXPANSION_SPEED.min)+t.EXPANSION_SPEED.min,waveIntensity:n.waveIntensity||this.rng.random()*(t.WAVE_INTENSITY.max-t.WAVE_INTENSITY.min)+t.WAVE_INTENSITY.min,maxRadius:n.maxRadius||i*(this.rng.random()*(t.MAX_RADIUS_MULTIPLIER.max-t.MAX_RADIUS_MULTIPLIER.min)+t.MAX_RADIUS_MULTIPLIER.min),glowIntensity:n.glowIntensity||this.rng.random()*(t.GLOW_INTENSITY.max-t.GLOW_INTENSITY.min)+t.GLOW_INTENSITY.min,cosmicOriginTime:n.cosmicOriginTime||f,timeSpeed:n.timeSpeed||this.rng.random()*(t.TIME_SPEED.max-t.TIME_SPEED.min)+t.TIME_SPEED.min,seed:e},this.cosmicOffset=e%100*.1,this.group=new w,this.createConcentricRings()}createConcentricRings(){const i=this.params.ringCount,n=this.params.maxRadius;for(let e=0;e<i;e++){const s=e/(i-1),r=this.planetRadius*1.02+(n-this.planetRadius*1.02)*Math.pow(s,.8),o=128,l=[];for(let a=0;a<=o;a++){const d=a/o*Math.PI*2,E=Math.cos(d)*r,x=Math.sin(d)*r,R=this.planetRadius*.005,T=Math.sin(d*7+e*2)*.5*.01*this.planetRadius,y=R+T;l.push(new N(E,y,x))}const c=new O().setFromPoints(l),g=new Float32Array(o+1),I=new Float32Array(o+1),h=new Float32Array(o+1);for(let a=0;a<=o;a++)g[a]=e*.6+a/o*Math.PI*4,I[a]=(r-this.planetRadius*1.02)/(n-this.planetRadius*1.02),h[a]=(e*7+a*3)%100/100;c.setAttribute("phase",new p(g,1)),c.setAttribute("distance",new p(I,1)),c.setAttribute("randomOffset",new p(h,1));const S=new A({transparent:!0,depthWrite:!1,depthTest:!0,blending:C,side:M,uniforms:{time:{value:0},color:{value:new v(this.params.color[0],this.params.color[1],this.params.color[2])},expansionSpeed:{value:this.params.expansionSpeed},waveIntensity:{value:this.params.waveIntensity},glowIntensity:{value:this.params.glowIntensity},ringIndex:{value:e},totalRings:{value:i}},vertexShader:`
