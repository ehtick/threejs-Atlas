...
class PlanetTypeTranslators:
    ...
    def translate_crystalline(self, planet_radius: int, rng: random.Random, 
                             seed: int, planet_name: str) -> Dict[str, Any]:
        """Translate Crystalline planet elements - crystal worlds with cyan atmospheres and crystalline formations"""
        center_x, center_y = 200, 200  # Pillow center coordinates
        
        # Generate crystalline atmospheric clouds (cyan/blue crystal-like atmosphere)
        num_clouds = rng.randint(8, 14)  # Moderate crystalline atmospheric activity
        clouds = []
        for i in range(num_clouds):
            cloud_radius = rng.randint(25, 45)  # Medium to large crystal clouds
            max_offset = planet_radius - cloud_radius
            cloud_x = center_x + rng.randint(-max_offset, max_offset)
            cloud_y = center_y + rng.randint(-max_offset, max_offset)
            ...
            # Crystalline clouds - cyan/blue crystalline colors with transparency
            cloud_colors = [
                [0.4, 0.8, 1.0, 0.8],       # Bright cyan crystalline
                [0.2, 0.7, 0.9, 0.75],      # Medium cyan-blue
                [0.5, 0.9, 1.0, 0.7],       # Light cyan crystalline
                [0.1, 0.6, 0.8, 0.85],      # Deep cyan-blue crystal
                [0.3, 0.75, 0.95, 0.8],     # Standard crystalline cyan
            ]
            ...
            clouds.append({
                ...
                "color": rng.choice(cloud_colors),
                "type": "cloud",
                "seed": f"{planet_name}_crystalline_cloud_{i}"
            })
        
        # Generate crystalline formations (cyan crystal patches)
        num_crystals = rng.randint(10, 18)  # Many crystal formations
        green_patches = []
        ...
        for i in range(num_crystals):
            ...
            # Size distribution - varied crystal formation sizes
            if i < 4:
                # Large crystal clusters
                size = rng.uniform(0.2, 0.35)
            elif i < 10:
                # Medium crystal formations
                size = rng.uniform(0.12, 0.25)
            else:
                # Small crystal outcrops
                size = rng.uniform(0.06, 0.15)
            
            # Crystalline colors - various cyan/blue crystal tones
            crystalline_color_choices = [
                [0.0, 0.8, 1.0],   # Pure cyan crystal
                [0.2, 0.9, 1.0],   # Light cyan crystal
                [0.1, 0.7, 0.9],   # Medium cyan-blue crystal
                [0.0, 0.6, 0.8],   # Deep cyan crystal
                [0.3, 0.85, 0.95], # Pale cyan crystal
                [0.05, 0.75, 0.85],# Darker cyan crystal
            ]
            
            green_patches.append({
                "position_3d": position_3d,
                "size": size,
                "color": rng.choice(crystalline_color_choices) + [rng.uniform(0.85, 0.95)],  # High opacity for crystal visibility
                "sides": rng.randint(6, 12),  # Angular crystal shapes (fewer sides for geometric look)
                "height": rng.uniform(0.03, 0.08),  # Higher elevation for crystal protrusions
                "crystalline_structure": True,  # Flag for crystalline formations
                "refraction_index": rng.uniform(0.8, 1.0)  # Crystal light properties
            })
        
        return {
            "type": "crystalline",
            "clouds": clouds,  # AtmosphereClouds will use this
            "green_patches": green_patches,  # LandMasses will use this for crystal formations
            "debug": {
                "original_planet_radius": planet_radius,
                "center_x": center_x, "center_y": center_y,
                "cloud_count": num_clouds,
                "crystal_formation_count": num_crystals,
                "largest_crystal_size": max([cf["size"] for cf in green_patches]) if green_patches else 0,
                "crystal_coverage": "cyan_crystalline_formations"
            }
        }
